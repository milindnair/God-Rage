  ...
  return (
    <div className=" mb-5 mt-3 h-full  overflow-y-scroll no-scrollbar">
      <div className="flex justify-between items-center">
        <h2 className='text-xl nunito_sans'>Chat History</h2>
        <Tooltip content="Create New Chat" className='text-black font- '>
          <Button isIconOnly className="hover:bg-white/10 " color="danger" aria-label="Like" onClick={handleNewChat}>
      ...
      {chathistory.length === 0 ? (
        <Card className="bg-blue-500 p-4 px-4 py-2 mb-1">
          <h2 className='text-md nunito_sans text-white '>No chats yet</h2>
          <h2 className='text-md nunito_sans text-white'>Your chat history will appear here</h2>
        </Card>
      ) : (
        chathistory.map((chat, index) => (
         <Card
            key={index}
            isPressable
            className="w-full bg-white/10 text-white px-4 py-3 mb-1.5 rounded-l-lg rounded-r-lg transform transition-transform duration-300 hover:scale-105 hover:-translate-y-2 my-3 min-h-[15%] max-h-[20%]"
            onClick={() => handleChatClick(chat.title)}
          >
            <h2 className='text-md text-left poppins'>{chat.title}</h2>
          </Card>
        ))
      )}
    </div>
